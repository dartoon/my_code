...
def gen_fit_ingredient(seq,cut_frame=120, subbkl='n'):
    '''
    ...
    '''
    seq = seq  # the sequence of the ID, if 2 seq = 2-1 = 1
    filename = glob.glob('../IMAGES/{0}-*fits'.format(seq+1))[0]
...
#    file_header1 = fitsFile[1].header
    ...
    data_info_file= '../sdss_hsc_match_zgt0.2_lt0.5.asc'
...
#    print "the pixel of the QSO:", QSO_x, QSO_y 
    ...
    PSF_name = glob.glob('../PSF*/{0}-*'.format(seq+1))
#    print "PSF_name", PSF_name
    PSF = pyfits.getdata(PSF_name[0])
    ...
    return QSO_im, err_map, PSF, pix_scale, zp, qso_ID, center, [load_RA, load_DEC], filename
