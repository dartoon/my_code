...
#    mpi = False  # MPI possible, but not supported through that notebook.
    ...
    if dump_result == True:
        if flux_ratio_plot==True and no_MCMC==False:
            ...
            trans_paras = []
        picklename= tag + '.pkl'
        pickle.dump([source_result, image_host, ps_result, image_ps, samples_mcmc, param_mcmc, trans_paras, chain_list, param_list], open(picklename, 'wb'))
...
#        image_host.append(image_host_i)  
    ...
    lensPlot = LensModelPlot(kwargs_data, kwargs_psf, kwargs_numerics, kwargs_model, lens_result, source_result,
                             lens_light_result, ps_result, arrow_size=0.02, cmap_string="gist_heat")
    if pso_diag == True and pltshow == 1:
        import lenstronomy.Plots.output_plots as out_plot
        for i in range(len(chain_list)):
            if len(param_list[i]) > 0:
                f, axes = out_plot.plot_chain(chain_list[i], param_list[i])    
...
#        print "total flux", image_host_i.sum()
    ...
    if dump_result == True:
        if flux_corner_plot==True and no_MCMC==False:
            trans_paras = [source_params[2], mcmc_new_list, labels_new]
        else:
            trans_paras = []
        picklename= tag + '.pkl'
        pickle.dump([source_result, image_host, samples_mcmc, param_mcmc, trans_paras, chain_list, param_list], open(picklename, 'wb'))
