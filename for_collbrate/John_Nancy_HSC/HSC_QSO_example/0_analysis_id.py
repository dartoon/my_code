...
from flux_profile import profiles_compare, flux_profile, fit_bkg_as_gaussian
...
image_folder = './image/'
run_MCMC = True
# Rather than set fit_frame_size as 71, the script will start from the smallest frame size to cutout.
#fit_frame_size = 71
...
qso_x, qso_y, qso_flux, qso_mag, host_qso_center_mismatch(arcsec), qso_local_bkg_light(flux/pixel)\
\n")
...
for cut_size in size_list:
    ct = (len(QSO_im_list[0])-cut_size)/2
    cut_data = QSO_im_list[0][ct:-ct,ct:-ct]
    edge_data = np.concatenate([cut_data[0,:],cut_data[-1,:],cut_data[:,0], cut_data[:,-1]])
    gauss_mean, gauss_1sig = fit_bkg_as_gaussian(edge_data, ifplot=pltshow, return_sig=1)
...
for k in run_list:#len(band_seq)):
    ...
    if isinstance(QSO_bkg_list[k],str): 
        ...
        ascii.write("{0}\n".format(round(np.mean(QSO_bkg_list[k][ct:-ct,ct:-ct]),5)))
    ascii.close()
