...
for i in range(len(namelist)):
    ...
    fr = fr                           # frame size
    psf = pyfits.getdata('{0}.fits'.format(psfname))  # Which PSF to use
    block_id = mask
    ...
    yn = 131
    fr = int(fr)
    neighbor_x = (fr-xn)*pix_sz #offset in arcsec; object on the left side should have center_x as positive, since the east are on the left direction.
    neighbor_y = -(fr-yn)*pix_sz #offset in arcsec
    print xn, neighbor_x, yn, neighbor_y
    
    agn_image = pyfits.getdata('{0}_{1}_cutout.fits'.format(ID,fr))
    agn_stdd = pyfits.getdata('{0}_{1}_stdd.fits'.format(ID,fr))
    ...
    kwargs_lower_source.append({'e1': -0.5, 'e2': -0.5, 'R_sersic': 0.04, 'n_sersic': 0.5, 'center_x': -0.04, 'center_y': -0.04})
    kwargs_upper_source.append({'e1': 0.5, 'e2': 0.5, 'R_sersic': 30., 'n_sersic': 6,'center_x': 0.04, 'center_y': 0.04})

    #neighbor
    fixed_source.append({})  
    kwargs_source_init.append({'R_sersic': 1, 'n_sersic': 4., 'e1': 0., 'e2': 0., 'center_x': neighbor_x, 'center_y': neighbor_y})
    kwargs_source_sigma.append({'n_sersic': 0.5, 'R_sersic': 0.5, 'e1': 0.1, 'e2': 0.1, 'center_x': 0.1, 'center_y': 0.1})
    kwargs_lower_source.append({'e1': -0.5, 'e2': -0.5, 'R_sersic': 0.04, 'n_sersic': 0.5, 'center_x': neighbor_x-0.1, 'center_y': neighbor_y-0.1})
    kwargs_upper_source.append({'e1': 0.5, 'e2': 0.5, 'R_sersic': 30., 'n_sersic': 6,'center_x': neighbor_x+0.1, 'center_y': neighbor_y+0.1})
    ...
    kwargs_lower_ps.append({'ra_image': [-0.04], 'dec_image': [-0.04]})
    kwargs_upper_ps.append({'ra_image': [0.04], 'dec_image': [0.04]})
