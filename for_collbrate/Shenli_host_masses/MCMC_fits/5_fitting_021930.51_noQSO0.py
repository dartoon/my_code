...
import copy
from mask_objects import find_loc_max, mask_obj

image_ID ='021930.51-055643.0' 
image_RA = 34.87710622 #- 0.165*10/3600
image_DEC = -5.945285416 #- 0.165*10/3600
...
for k in [band_run_list[0]]:  #['G', 'R', 'I', 'Z', 'Y']
    ...
    x, y = find_loc_max(QSO_img, neighborhood_size=4)
...
for k in run_list:#len(band_seq)):
    ...
    _ = transfer_to_result(data=QSO_img, pix_sz = pix_scale,  
                                ...
                                zp=zp_list[k], fixcenter=fixcenter,ID=image_ID+band_seq[k], QSO_msk = QSO_msk, tag=tag, plot_compare = plot_compare)
    #
