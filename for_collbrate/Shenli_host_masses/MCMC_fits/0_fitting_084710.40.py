    ...
    if band_seq[k] == 'I' or fix_on_I_band != 1: 
        ...
        if len(obj) >= 1:
            for i in range(len(obj)):
                ...
                kwargs_source_sigma.append({'n_sersic': 0.5, 'R_sersic': 0.5, 'e1': 0.1, 'e2': 0.1, 'center_x': 0.1, 'center_y': 0.1})
                kwargs_lower_source.append({'e1': -0.5, 'e2': -0.5, 'R_sersic': obj[i][1] * pix_scale/5, 'n_sersic': 0.5, 'center_x': -obj[i][0][0]*pix_scale-1.5*pix_scale, 'center_y': obj[i][0][1]*pix_scale-1.5*pix_scale})
                kwargs_upper_source.append({'e1': 0.5, 'e2': 0.5, 'R_sersic': obj[i][1] * pix_scale * 2, 'n_sersic': 7., 'center_x': -obj[i][0][0]*pix_scale+1.5*pix_scale, 'center_y': obj[i][0][1]*pix_scale+1.5*pix_scale})
