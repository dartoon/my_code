...
pltshow = 0 #input("Do you want to plot while fitting?input 0 means no, input others means yes. \n")
if pltshow == 0:
    plt.ion()
...
Reff(arcsec), n_sersic, galaxy_q, Kron_radius(arcsec), galaxy_Kron_flux, galaxy_Kron_mag, concentration, reduced_Chisq \
\n")
    ascii.close()
...
for i in range(len(band_seq)):
        ...
        galaxy_im, err_map, PSF, pix_scale, zp, galaxy_fr_center, fr_c_RA_DEC = gen_fit_id(image_folder,
                                                                                        galaxy_RA, galaxy_DEC, filename_list[i], cut_frame=120,pltshow=0, subbkl = 'y')
...
if reg_file != []:
    ...
    for k in run_list:
            ...
            print "the number of nearby objs:", len(obj)
obj = [obj[2]]
obj.append([(-25,15),4,1])
#plot the image with obj position
...
for k in run_list:#len(band_seq)):
        ...
        source_result, image_host, error_map, reduced_Chisq=fit_galaxy(galaxy_img, psf_ave=psf, psf_std = None,
                                                      ...
                                                      galaxy_std =galaxy_std, tag=tag, deep_seed= deep_seed, pltshow=pltshow, return_Chisq=True)
        if band_seq[k] == 'I':
        ...
        ascii.write("{0} {1} {2} {3} {4} {5} {6} {7}\n".format(result['R_sersic'],result['n_sersic'],result['q'],result['Kron_radius'], result['Kron_flux'],
                    result['Kron_mag'], result['concentration'], result['reduced_Chisq'],))
        ascii.close()
        t2 = time.time()
        time_sp = t2-t1
        time_ave = time_sp/(k + 1)
        time_total = time_ave * 5
