...
\tablewidth{0pt}
\tablecaption{{\bf Illustration of the mock GW catalog}} 
\tablehead{ 
\colhead{Object ID} &
\colhead{\mone}&
...
\tablecomments{The catalog of simulated thousand BBH inspiral events is used to test the inference of the BHMF from the data attainable in forthcoming next generation GW detector -- the ET. The reported values are the medians, with errors corresponding to the 16th and 84th percentiles, {\bf assuming $\alpha=1.6$, $M_{min}~=~5M_{\odot}$, $M_{max}~=~50M_{\odot}$. Note that this mock data is re-simulated every time in each realization}.
}
\end{deluxetable}
...
\caption{
\blue{
Assuming a set of \dl\ and \cmass\ following the Log-Normal distribution, we randomly produce the the corresponding histogram of the $1/\eta$ distribution. The result shows that the skewed distribution could be well described by a Log-Normal distribution with $\sigma = -\log(\eta_{\rm median})/3$.
}
}
\label{fig_eta}
\end{figure}
