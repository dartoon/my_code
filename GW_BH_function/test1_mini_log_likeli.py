...
if use_method == 'med':
    filename = 'test2_0_med(ave0-1)_un{0}.txt'.format(int(m_noise_level*100))   #obsun means the std are from the obs data rather than true data
#elif use_method == 'exp':
...
def point_Chisq(para, m1_obs,m1_sig):
    ...
    if 1.1 < a < 3 and 50 < mbh_max < 100 and 2 < mbh_min < 8:
        poss_m1_i = likelihood(m1_obs,m1_sig, a=a, mbh_max=mbh_max, mbh_min=mbh_min, use_method='med')
