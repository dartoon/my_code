...
We performed the Monte Carlo approach to simulate the measurements of BH binaries GW signals as detected by ET, including the BH masses and their luminosity distance (\dl). Following the previous works, we consider the mass of  primary black hole in each binary and model the BHMF as a function of the power law with slope parameter as $\alpha$. We take into account the bias that would be raised by the uncertainty of the measurement and the selection effect and find that a thousand GW events measure by ET ($\sim1\%$ amount of the yearly detection rate) could measure the $\alpha$ to $3\%$ accuracy level. Furthermore, we investigate a scenario, by the first time, that  $\alpha$ is evolving with redshift as $\alpha(z) = \alpha_0 + \alpha_1\frac{z}{1+z}$. Our result is that, taking the \dl\ as redshift estimator, with a thousand GW events one is capable of obtaining an unbiased $\alpha_1$ at the uncertainty level of $\pm0.5$. The ability of known the evolution scenario of BHMF would help to understand the origin of the BHs and how BH binaries formed at the different stage of the Universe.
