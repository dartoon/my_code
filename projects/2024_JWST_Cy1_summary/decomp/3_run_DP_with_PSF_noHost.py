...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar  5 21:43:32 2024
...
# for idx in [1]:
    ...
    for ct, PSFs_file in enumerate(PSF_library_DP_files):
        ...
        for i in range(len(PSF_list_clean)):
            ...
            # if_run = glob.glob(save_folder+'fit_run_{0}_idx{1}_psfidx{2}_{3}_psfsf{4}_noHost.pkl'.format(filt, idx, psf_idx, i, point_source_supersampling_factor))
            # if if_run != []:
            #     continue
            ...
            pickle.dump(fit_run , open(save_folder+'fit_run_{0}_idx{1}_psfidx{2}_{3}_psfsf{4}_noHost.pkl'.format(filt, idx, psf_idx, i, point_source_supersampling_factor), 'wb'))
