...
for add_cond in ['', '_fixn1']:
    ...
    if add_cond == '':
        fit_run_free_n = fit_run_list[sort_Chisq[0]]    
    elif add_cond == '_fixn1':
...
if rerun == True:
    ...
    fit_run_fix_n4 = FittingProcess(fit_sepc, savename = target_id)
    fit_run_fix_n4.run(algorithm_list = ['PSO','PSO', 'PSO'], fitting_level=['norm','deep', 'deep'])
    fit_run_fix_n4.plot_final_qso_fit(target_ID =target_id)
    pickle.dump(fit_run_fix_n4 , open('./material/fit_run_{0}_idx{1}_psfsp{2}_fixn4.pkl'.format(filt, idx, point_source_supersampling_factor), 'wb'))
    
else:
    file = './material/fit_run_{0}_idx{1}_psfsp{2}_fixn4.pkl'.format(filt, idx, point_source_supersampling_factor)
    fit_run_fix_n4 = pickle.load(open(file,'rb'))
