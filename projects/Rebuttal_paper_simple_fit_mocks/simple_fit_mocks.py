...
H0_true = 70.65595
#noise_seed = 2
seed = [211, 212, 213, 214, 215, 216][0]
...
#        for i in range(len(samples_mcmc)):
    ...
    import os
    if os.path.exists('./MCsteps{0}'.format(steps))==False:
        os.mkdir('./MCsteps{0}'.format(steps))
    
    if fix_gamma:
        plot.savefig("MCsteps{0}/corner_plot_SIE#{1}.pdf".format(steps, seed-210))
        datafile = 'MCsteps{0}/run_result_SIE.txt'.format(steps)
        picklename = 'MCsteps{0}/sampler_results_SIE#{1}.pkl'.format(steps, seed-210)
    else:
        plot.savefig("MCsteps{0}/corner_plot_SPEMD#{1}.pdf".format(steps, seed-210))                     
        datafile = 'MCsteps{0}/run_result_SPEMD.txt'.format(steps)
        picklename = 'MCsteps{0}/sampler_results_SPEMD#{1}.pkl'.format(steps, seed-210)
    plt.close()
