...
#H0_lists = []
#for i in range(1,7):
#    result = pickle.load(open('Final-MCMC_4000_steps/sampler_results_SIE#{0}.pkl'.format(i),'rb'))
#    _, trans_result = result
#    mcmc_new_list, labels_new = trans_result
#    H0_list = mcmc_new_list[:,-1]
#    H0_lists.append(H0_list) 
#pickle.dump([H0_lists], open('H0_SIE.pkl', 'wb'))         
#H0_lists = []
#for i in range(1,7):
#    result = pickle.load(open('Final-MCMC_4000_steps/sampler_results_SPEMD#{0}.pkl'.format(i),'rb'))
#    _, trans_result = result
#    mcmc_new_list, labels_new = trans_result
#    H0_list = mcmc_new_list[:,-1]
#    H0_lists.append(H0_list) 
#pickle.dump([H0_lists], open('H0_SPEMD.pkl', 'wb'))         

H0_true = 70.65595
model = 'SIE'
#model = 'SPEMD'
name = {'SIE':'SIE', 'SPEMD': "Power - law"}
H0 = pickle.load(open('Final_before_submitpaper/H0_{0}.pkl'.format(model),'rb'))[0]
