...
jwst_all_filenames = glob.glob(folder+'*{0}*_rmbkg.fits'.format(filt))  #For NIRCam
...
for filt in [filt]:
    ...
    if _fitsFile[0].header['CHANNEL'] == 'LONG':
        radius = 45
    elif _fitsFile[0].header['CHANNEL'] == 'SHORT':
        radius = 45
    ...
    if _fitsFile[0].header['CHANNEL'] == 'LONG':
        ...
        gain_value = 1.8
        expsize = 1
    # exp_map = _fitsFile[2].data #exp * wht/wht.max() / flux_mjsr * gain_value
    exp_map = exp / flux_mjsr * gain_value
    ...
    data_process.generate_target_materials(radius=radius * expsize, 
                                           create_mask = False, nsigma=1.5, 
