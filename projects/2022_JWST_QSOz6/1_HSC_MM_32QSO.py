...
# infers  = np.loadtxt('../../../my_code/projects/Plot_MM_relation/sdss_quasar_decomposition_v1.txt', dtype=str)
# IDs_ = infers[:, 0]
...
# flags_  = np.loadtxt('../../../my_code/projects/Plot_MM_relation/sdss_quasar_decomposition_v1_catalog_flag.txt', dtype=str)
# flags = flags_[:,0]
...
#%%
plt.scatter([10.65,11.36],[8+np.log10(2.9), 9+np.log10(3.1)],c='red',
            s=1020, marker="*",zorder=100, edgecolors='k', alpha = 1)

#%%
ALMA = np.loadtxt('other_files/Izumi+21_fullsample.txt')
ALMA_MBH, ALMA_dynM = ALMA[:,0],  ALMA[:,1]
plt.scatter( np.log10(ALMA_dynM),np.log10(ALMA_MBH) ,c='blue',
            s=520, marker="*",zorder=99, edgecolors='k', alpha = 1)

# plt.scatter([10+np.log10(1.3), 10+np.log10(2.0), 10+np.log10(29)],
#             [8+np.log10(7.1), 8+np.log10(1.1), 8+np.log10(11)],c='blue',
#             s=1020, marker="*",zorder=100, edgecolors='k', alpha = 1)
...
JWST = mlines.Line2D([], [], color='red', ls='', marker='*', markersize=25,markeredgecolor='k')
ALMA = mlines.Line2D([], [], color='blue', ls='', marker='*', markersize=25,markeredgecolor='k')
plt.legend([Bkc,Hkc,SS13,ding_sample, JWST, ALMA],[
'Local by Bennert+11',\
"Local by H&R",
"0.3<quasars<2",
...
'JWST at z >6',\
'Izumi + 2021'
],scatterpoints=1,numpoints=1,loc=3,prop={'size':18,'family': 'Arial'},ncol=2)
