...
filt = 'F356W'
idx = 3
files = glob.glob(run_folder+'fit_material/data_process_idx{1}_*{0}*FOVpsf*.pkl'.format(filt,idx))
files.sort()
collect_info = []
...
for count in range(len(collect_info)):
    ...
    for ct in [2, 3, 5]:
        ...
        if ct >8:
            ct = 'all'
        pickle.dump(_data_process , open(run_folder+'fit_material/'+'data_process_idx{0}_{2}_IncludeOtherID_CombPsfsNO_{1}.pkl'.format(idx, ct, filt), 'wb'))
