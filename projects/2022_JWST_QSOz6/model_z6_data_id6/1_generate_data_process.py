...
for filt in [filt]:
    ...
    target_pos = [RA, Dec]
    if filt == 'F150W':
        target_pos = np.array([5546, 2944])
        pos_type = 'pixel'
    data_process = DataProcess(fov_image = fov_image, target_pos = target_pos, #The final cut center is dete
                                   pos_type = pos_type, header = header,rm_bkglight = False, 
                                   if_plot=False, zp = zp, exptime= exp_map, 
                                   fov_noise_map = None)
    
    data_process.generate_target_materials(radius=radius * expsize, create_mask = False, nsigma=2., 
                                            cut_kernel = None, if_select_obj=False,
                                          exp_sz= 1.2, npixels = 30 * expsize, if_plot=False)
