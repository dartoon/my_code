...
folder = '20230115' #The one show in paper
...
jwst_filt_id = {'F115W': '352', 'F150W': '353', 'F200W': '354', 
           ...
           'F250M':'358', 'F300M':'359', 'F460M':'364', 'F480M':'365'}
...
for i, fid in enumerate(filt_id[::-1]):
    f_fil = np.loadtxt('../../../../template/gsf_temp/filter/{0}.fil'.format(fid))
    top = f_50.max()
    f_fil[:,2] = f_fil[:,2]/f_fil[:,2].max() * (ymax-ymin) * 0.4
...
for i, filt in enumerate(['F115W', 'F200W', 'F250M', 'F300M', 'F444W', 'F480M']):
    ...
    top = f_50.max()
    f_fil[:,2] = f_fil[:,2]/f_fil[:,2].max() * (ymax-ymin) * 0.4
    plt.plot(f_fil[1:,1]/10000., f_fil[1:,2], '--', label='{0}'.format(ivd[fid]),linewidth=3, color= to_rgba[sec_color[i]])
...
plt.text( (xmax-xmin)*0.07, (ymax-ymin)*0.88, 'age = {0:.2f} Gyr'.format(age, age_l, age_h), fontsize=17)
