...
for ID in [24]:
# for ID in range(24,48):
# for ID in range(25,50):
    np.random.seed(seed = seed)
    name = keys[1] #!!! ID of target
    host_flux_ratio = 1 #np.random.uniform(0.07,0.9) #!!!
    host_Reff_kpc = np.random.uniform(6,6.1)   #Host effective radius, unit: Kpc #!!!
    source_id = ID #!!!
    ...
    if filt=='f356w':
        ...
        fov_cut_idx = 1 #np.random.randint(0,len(data_sbs))
    ...
    for i in range(500):
        ...
        try:
            res, segm_map = detect_obj(cut1, segm_map=True, auto_sort_center=False)
    ...
    data_process.generate_target_materials(radius=len(project_gal_img)/2, create_mask = False, nsigma=2.8, if_select_obj=False,
                                          exp_sz= 1.2, npixels = 55, if_plot=False)
    ...
    for psf, name in [[psf_true,'same_psf']]:
    # for psf, name in [[psf_model,'diff_psf']]:    
        ...
        print('inferred galaxy flux, mag, Re (arcsec), n_sersic):\n\t', round(fit_run.final_result_galaxy[0]['flux_within_frame'],2), 
              round(fit_run.final_result_galaxy[0]['magnitude'],2), 
              round(fit_run.final_result_galaxy[0]['R_sersic'],2),
              round(fit_run.final_result_galaxy[0]['n_sersic'],3))
        fit_run.plot_final_galaxy_fit(target_ID=name[:5]+'_HostRto_'+str(round(host_flux_ratio,3)), save_plot=True, 
                                   show_plot=if_plot)
    #%%
