...
if idx == 1:
    fit_files = glob.glob(run_folder+'*fit_material_super2/fit_run*_idx{0}_{1}_*FOV*.pkl'.format(idx, filt))#+\
elif idx == 0:
    fit_files = glob.glob(run_folder+'*fit_material_super2/fit_run_idx{0}_{1}_*FOV*.pkl'.format(idx, filt))#+\
fit_files.sort()
fit_run_list = []
...
chisqs_PSF = np.array([fit_run_list[i].reduced_Chisq for i in range(len(fit_run_list))])  #!!!
...
for seed in range(80,90):
    ...
    for fit_psf in range(4):
        ...
        host_highres = imageModel.image(kwargs_lens_light=kwargs_host, 
                                        unconvolved=False)
        data_mock = copy.deepcopy(data)
        data_mock[ pos[1]-rad:pos[1]+rad+1, pos[0]-rad:pos[0]+rad+1] += host_highres
        if idx == 0:
            data_mock[ pos[1]-rad:pos[1]+rad+1, pos[0]-rad:pos[0]+rad+1] += psf[25:-25,25:-25] / np.sum(psf[25:-25,25:-25]) * fit_run_.final_result_ps[0]['flux_within_frame']
        elif idx == 1:
            data_mock[ pos[1]-rad:pos[1]+rad+1, pos[0]-rad:pos[0]+rad+1] += psf[20:-20,20:-20] / np.sum(psf[20:-20,20:-20]) * fit_run_.final_result_ps[0]['flux_within_frame']
        ...
        plot_fit_name = 'sim_result_PSF_super/sim_idx{3}_{4}_seed{0}BasedPSF{1}_FitBasedPSF{2}_super1'.format(seed,sim_psf,fit_psf,idx, filt)
        ...
        plot_fit_name = 'sim_result_PSF_super/sim_idx{3}_{4}_seed{0}BasedPSF{1}_FitBasedPSF{2}_super2'.format(seed,sim_psf,fit_psf,idx, filt)
        fit_sepc.kwargs_numerics['point_source_supersampling_factor'] = 2
        fit_run_super = FittingProcess(fit_sepc, savename = plot_fit_name, fitting_level='norm')
        fit_run_super.run(algorithm_list = ['PSO','PSO', 'PSO'], fitting_level=['norm','deep', 'deep'])
        fit_run_super.plot_final_qso_fit()
        fit_run_super.dump_result()
        
        plot_fit_name = 'sim_result_PSF_super/sim_idx{3}_{4}_seed{0}BasedPSF{1}_FitBasedPSF{2}_super4'.format(seed,sim_psf,fit_psf,idx, filt)
        fit_sepc.kwargs_numerics['point_source_supersampling_factor'] = 4
        fit_run_super = FittingProcess(fit_sepc, savename = plot_fit_name, fitting_level='norm')
        fit_run_super.run(algorithm_list = ['PSO','PSO', 'PSO'], fitting_level=['norm','deep', 'deep'])
        fit_run_super.plot_final_qso_fit()
        fit_run_super.dump_result()
        # print(round(fit_run_.final_result_galaxy[0]['magnitude'],3), round(fit_run.final_result_galaxy[0]['magnitude'],3), 
        #       round(fit_run_super.final_result_galaxy[0]['magnitude'],3) )
    #%% recombin use top-2 PSF:
    # Load top PSF result from the overall fittings
    fit_files = glob.glob('sim_result_PSF_super/sim_idx{1}_{2}_seed{0}B*_super1.pkl'.format(seed,idx,filt))#+\
    fit_files.sort()
    fit_run_list = []
    for i in range(len(fit_files)):
        fit_run_list.append(pickle.load(open(fit_files[i],'rb')))
    ...
    fit_sepc.build_fitting_seq()
    fit_sepc.kwargs_params['lens_light_model'][0] = [fit_run_.kwargs_result['kwargs_lens_light'][0]]
    plot_fit_name = 'sim_result_PSF_super/sim_idx{2}_{3}_seed{0}BasedPSF{1}_FitBasedCombPSF_super1'.format(seed,sim_psf,idx,filt)
    fit_run = FittingProcess(fit_sepc, savename = plot_fit_name, fitting_level='norm')
    fit_run.run(algorithm_list = ['PSO','PSO', 'PSO'], fitting_level=['norm','deep', 'deep'])
    fit_run.plot_final_qso_fit()
    fit_run.dump_result()
    
    fit_files = glob.glob('sim_result_PSF_super/sim_idx{1}_{2}_seed{0}B*_super2.pkl'.format(seed,idx,filt))#+\
    fit_files.sort()
    fit_run_list = []
    for i in range(len(fit_files)):
        fit_run_list.append(pickle.load(open(fit_files[i],'rb')))
    ...
    fit_sepc.kwargs_params['lens_light_model'][0] = [fit_run_.kwargs_result['kwargs_lens_light'][0]]
    fit_sepc.kwargs_numerics['point_source_supersampling_factor'] = 2
    plot_fit_name = 'sim_result_PSF_super/sim_idx{2}_{3}_seed{0}BasedPSF{1}_FitBasedCombPSF_super2'.format(seed,sim_psf,idx,filt)
    fit_run = FittingProcess(fit_sepc, savename = plot_fit_name, fitting_level='norm')
    fit_run.run(algorithm_list = ['PSO','PSO', 'PSO'], fitting_level=['norm','deep', 'deep'])
    fit_run.plot_final_qso_fit()
    fit_run.dump_result()
    
    fit_files = glob.glob('sim_result_PSF_super/sim_idx{1}_{2}_seed{0}B*_super4.pkl'.format(seed,idx,filt))#+\
    fit_files.sort()
    fit_run_list = []
    for i in range(len(fit_files)):
        fit_run_list.append(pickle.load(open(fit_files[i],'rb')))
    ...
    fit_sepc.kwargs_params['lens_light_model'][0] = [fit_run_.kwargs_result['kwargs_lens_light'][0]]
    fit_sepc.kwargs_numerics['point_source_supersampling_factor'] = 4
    plot_fit_name = 'sim_result_PSF_super/sim_idx{2}_{3}_seed{0}BasedPSF{1}_FitBasedCombPSF_super4'.format(seed,sim_psf,idx,filt)
    fit_run = FittingProcess(fit_sepc, savename = plot_fit_name, fitting_level='norm')
    fit_run.run(algorithm_list = ['PSO','PSO', 'PSO'], fitting_level=['norm','deep', 'deep'])
    fit_run.plot_final_qso_fit()
    fit_run.dump_result()
...
# fit_files = glob.glob('sim_result_PSF_super/sim_idx0_seed{0}B*.pkl'.format(seed))#+\
# fit_files.sort()
# fit_run_list = []
# for i in range(len(fit_files)):
#     fit_run_list.append(pickle.load(open(fit_files[i],'rb')))
