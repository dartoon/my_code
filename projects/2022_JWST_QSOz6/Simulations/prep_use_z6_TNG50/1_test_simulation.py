...
# TNG_folder = 'TNG50_img/'
# TNG_file = 'shalo_091-540258_v0_photo.fits'
import glob
# TNG_files = glob.glob('TNG50_img/*photo.fits')
TNG_files = glob.glob("TNG50_z6/Idealized/013/*7251*v?*fits")
TNG_files.sort()

seed = 0 #int(sys.argv[1])

# for ID in range(len(TNG_files)):
for ID in range(1):
    np.random.seed(seed = seed)
    name = keys[ID] #!!! ID of target
    # host_flux_ratio = np.random.uniform(0.07,0.9) #!!!
    host_flux_ratio = np.random.uniform(0.5,0.6) #!!!
    ...
    for i in range(200):
        ...
        apertures, segm_deblend, mask_apertures, tbl = detect_obj(cut1, nsigma=1, npixels=6, use_moments=False)
    ...
    fits = im['JWST_NIRCAM.'+img_filter].data
