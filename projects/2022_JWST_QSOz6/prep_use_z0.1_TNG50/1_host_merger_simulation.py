...
for ID in range(1):
    ...
    host_flux_ratio = 0.6 # np.random.uniform(0.07,0.95) #
    source_id =  1 #np.random.randint(0,len(TNG_ids)) #
    ...
    if filt=='f356w':
        ...
        pos_list = [[0,0], [0,4600], [4600,0], [4600, 4600], [250+0,11550+0], 
                    [250+0,11550+4600], [250+4600,11550+0], [250+4600, 11550+4600]]
        data_sbs = []
    ...
    for i in range(200):
        if filt=='f150w':
            pos = [int(np.random.uniform(600, 3800)), int(np.random.uniform(600, 3800))]  #
        elif filt=='f356w':
            pos = [int(np.random.uniform(600, 1400)), int(np.random.uniform(600, 1400))]  #
        rad1 = 100
    ...
    folder_save = 'results_find_host_merger/'
    ...
    data_process.generate_target_materials(radius=70, 
