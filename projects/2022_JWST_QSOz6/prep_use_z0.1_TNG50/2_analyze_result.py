...
res_files = glob.glob('results_extensive_sim/qsoID*filt_f15*pkl')
# res_files = glob.glob('results_extensive_sim/qsoID*filt_f35*pkl')
res_files.sort()
...
for file in res_files:
    ...
    res_reff.append(res['true_host_Reff_kpc'])
    TNG_ID.append(res['TNG_ID'].split('091-')[1][:6])
    if y == 'mag (inf$-$true)':
        ...
        res_scatter_diffPSF.append( res['inferred_R_sersic_diff_psf'] / res['true_host_Reff_arcsec'] )
        res_scatter_samePSF.append( res['inferred_R_sersic_same_psf'] / res['true_host_Reff_arcsec'] )
    ratio.append( res['true_host_flux_ratio'] )
    host_mag.append(res['true_host_mag'])

#%%
plt.figure(figsize=(11,11))
plt.scatter(host_mag, res_scatter_samePSF, label = 'use True PSF')
plt.scatter(host_mag, res_scatter_diffPSF, label = 'use diff PSF')
if y == 'mag (inf$-$true)':
    plt.ylim([-2.5, 2.5])
else:
    plt.ylim([0, 3])
plt.tick_params(labelsize=20)
plt.xlabel('Host magnitude',fontsize=27)
plt.ylabel(y, fontsize=27)
plt.legend(prop={'size':28})
plt.show()

# for i in range(len(res_scatter_diffPSF)):
#     if res_scatter_diffPSF[i]< -0.8:
#         print(TNG_ID[i])
