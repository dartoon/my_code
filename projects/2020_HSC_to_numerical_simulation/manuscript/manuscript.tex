...
%\red{We performed the 1000 times realizations for the comparison of each sample to estimate the uncertainties of the values in Table~\ref{tab:sum} arises by the random scatter. We find that the uncertainty level of the }
...
Black hole mass versus stellar mass for both the observational (small orange circles) and simulated (small colored circles) samples. Each row pertains to a particular simulation as labelled. The panels, from left to right, show different redshift bins. The black line in each panel indicates the local relation adopted by~\citet{Ding2020}. The background cloud (in green and yellow \red{with contours}) shows the intrinsic simulation number density before injecting random noise and applying selection effects. Only TNG100 is presented here since TNG300 presents very similar results \red{(see discussion section)}.
}
\end{figure*} 
