...
for i in [3,2,1,4,5,0]:
    ...
    if zs<1:
        Imag= Imag_list[zs]
        obs_dict, sim_dict = pickle.load(open(glob.glob('pickles/comb_zs{0}{1}_Imag_{2}_4.pkl'.format(zs,n_flag,Imag))[0],'rb'))
        # _, sim_int_dict = pickle.load(open(glob.glob('pickles/comb_zs{0}_Imag_{1}_nonoise.pkl'.format(zs,Imag))[0],'rb'))
        _, sim_int_dict = pickle.load(open(glob.glob('pickles/comb_zs{0}_Imag_{1}.pkl'.format(zs,21.5))[0],'rb'))
    elif zs>1:
    ...
    if zs == 0.5:
        _, sim_dict_ = pickle.load(open(glob.glob('pickles/comb_zs{0}{1}_Imag_{2}.pkl'.format(zs,n_flag,21.5))[0],'rb'))
        sim_dict['MBII'] = sim_dict_['MBII']
        _, sim_int_dict_ = pickle.load(open(glob.glob('pickles/comb_zs{0}{1}_Imag_{2}.pkl'.format(zs,n_flag,22.0))[0],'rb'))
        sim_int_dict['MBII'] = sim_int_dict_['MBII']
    ...
    axs[0].set_yticks([])
    axs[0].set_ylim([0,np.max(his_xy_[0])*1.2])
    ...
    his_xy = axs[ct+1].hist(off_int_sim, 
              density=True,histtype=u'step',linestyle=('dashed'),
...
plt.savefig('offset_dis_z{0}_different_selectImag.pdf'.format(zs), bbox_inches = "tight")
plt.show()
