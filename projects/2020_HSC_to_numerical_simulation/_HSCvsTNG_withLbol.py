...
#%%Load TNG sample:
filenames = glob.glob('TNG100_first_version/illustris_data/*') 
filenames.sort()
...
for i in [0]:    
    ...
    Stellar_Mass, BH_Mass, Eddington_ratio = np.load(filename)
...
plt.figure(figsize=(11.5,10))      
plt.scatter(logLbol  , BH_Mass  ,alpha=0.2)
plt.scatter(HSC_Lbol_overall, HSC_MBHs_overall,c='orange',alpha=0.1,zorder = 1)
# plt.scatter(BH_Mass, sdss_g_pointsource)
# plt.plot(np.linspace(5,10), np.linspace(5,10)*0 + abs_Mags)
