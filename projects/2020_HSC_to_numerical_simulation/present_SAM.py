...
HSC_galaxy_g_mag = infers[:,16].astype(np.float)
...
HSC_galaxy_g_mag = HSC_galaxy_g_mag[(HSC_z>(zs-0.1))*(HSC_z<(zs+0.1))]
...
Stellar_Mass_nois_nosl, BH_Mass_nois_nosl, agn_mag_reali, abs_Mags_break_reali, redshift_reali  = [], [], [], [], []
...
#%%
logM_mass_plot, g_galaxy_plot, BH_mass_plot, agn_mag_plot  = [], [], [], []
for i in range(5000):
    seed = np.random.uniform(0, c_weight[-1])
    j = np.sum(seed > c_weight)  #Random based on the weighting    
...
HSC_galaxy_g_abs_mag = HSC_galaxy_g_mag - 5*(np.log10(dl_HSC * 10**6)-1)   #Compare with and with r when z<0.5,  g when z >0.5
plt.scatter(HSC_Mstar, HSC_galaxy_g_abs_mag, c = 'orange', alpha=0.2)
...
plt.scatter(BH_mass_plot, agn_mag_plot,alpha=0.1)
