...
steller_files = glob.glob('esti_smass/2023022701*/SFH_*.fits')

steller_files.sort()
# print(len(steller_files))
for steller_file in steller_files:
    hdul = pyfits.open(steller_file)
    count = steller_file.split('2023022701')[1].split('/')[0]
    info = hdul[0].header 
    f = open( steller_file.split('SFH')[0]+'gsf_spec_header.txt',"r")
    string = f.read()
