...
size = 48  #For LW it is 56*4 and for SW it is 56*2
...
for i, filt in enumerate(run_filt_list[:]):
    ...
    if ratio>0.8:
        pos = np.where(img_org[:60,:40].max() == img_org)
        pos = [pos[0][0], pos[1][0]]
        cut_size = size *2
        x_bot = pos[0] - 30 
        y_bot = pos[1] - 12 
    elif ratio<0.8:
        pos = np.where(img_org[:75,:50].max() == img_org)
        pos = [pos[0][0], pos[1][0]]
        cut_size = size *4
        x_bot = pos[0] - 30 * 2 
        y_bot = pos[1] - 12 * 2
    img_org = img_org[x_bot:x_bot+cut_size,y_bot:y_bot+cut_size]
    # ct = int((len(img_org) - cut_size)/2)
