...
wht_max = {'F606W':349109.53, 'F814W':609417.25, 'F105W':11948740000, 'F125W':45840350000, 'F140W':858202000, 'F160W':45840350000.0}

for idx in range(54, len(lines)):
# for idx in [42]:
# for idx in range(58, 59):
    if idx in remove_id:
        continue
    line = lines[idx]
    # HST_all_files= glob.glob(HST_folder+'/egs_all_wfc3_ir_*_030mas_v1.9_drz.fits')  #For IR
    HST_all_files= glob.glob(HST_folder+'/egs_all_acs_wfc*_030mas_v1.9_drz.fits')  #For ACS
    ...
    for i in range(len(HST_all_files)):
        ...
        try:
            filt = header_HST['filter']
        except:
            filt = header_HST['FILTER1']
        if filt[0] != 'F':
            filt = header_HST['FILTER2']
            
        wht = pyfits.open(HST_all_files[i].replace('drz','wht'))[0].data
        exp = header_HST['EXPTIME']
        exp_map = exp * wht[int(data_process.target_pos[1]), int(data_process.target_pos[0])] / wht_max[filt]
        ...
        if fov_image_HST[int(data_process.target_pos[1]), int(data_process.target_pos[0])] == 0 :
            continue
        data_process.generate_target_materials(radius=radius, create_mask = False, skip=True,
                                                cut_kernel = cut_kernel, #nsigma=8,
                                                npixels = 30)
        ...
        data_process.generate_target_materials(radius=radius,cut_kernel=None, bkg_std = bkg_std, skip=True,
                                                create_mask = False)
    ...
    hold = input('Hold ... OK?\n')
    pickle.dump([[data_process_list, com_aper_l]], open('material/'+'data_process+apertures_{0}_ACS.pkl'.format(idx), 'wb'))
    
    import shutil
    shutil.copyfile('1_generate_data_process.py', 'backup_files/1_generate_data_process_idx{0}_ACS.py'.format(idx))
