...
#%%
spec1d = pyfits.open("./gsf_spec_1422.fits")  
...
# info = pyfits.open("./1234_PA00_sed.fits") #Have the blue model diamond.
plt.plot(wave, f)
...
f555w_fil = np.loadtxt('/Users/Dartoon/Astro/Packages/gsf/gsf/example/filter/213.fil')
f555w_fil[:,2] = f555w_fil[:,2]/f555w_fil[:,2].max()* array_spec[:,2].max()/6
plt.plot(f555w_fil[:,1]/10000., f555w_fil[:,2], label='WFC3-F814W response')

f791w_fil = np.loadtxt('/Users/Dartoon/Astro/Packages/gsf/gsf/example/filter/791.fil')
f791w_fil[:,2] = f791w_fil[:,2]/f791w_fil[:,2].max()* array_spec[:,2].max()/6
plt.plot(f791w_fil[:,1]/10000., f791w_fil[:,2], label='f491W response')

fnu = np.array([937.562, 19.055, 136.773])
lam = np.array([1.5396e+04, 5391.47, 7926.15])
yerr_l = np.array([8.635/937.562, 0.878/19.055, 7.562/136.773])
