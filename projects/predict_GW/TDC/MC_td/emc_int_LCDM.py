...
import time
time0=time.clock()
print np.shape(np.sum(twod_like(theta=[0.3,70], zl=data[:,1], y=data[:,3], err=data[:,4])[:,:-1]*ddz.T,axis=1))
...
#zs=data[:,0]
bnds = ((0, None), (0, None))
result = op.minimize(nll, (0.30, 70), method='SLSQP', bounds=bnds,args=(zl, y, err))
print result["x"]
'''
...
ndim, nwalkers = 2, 50
pos = [[0.30, 70] + 1e-4*np.random.randn(ndim) for i in range(nwalkers)]
...
import pickle
pickle.dump([samples,burn], open('samp_100.txt','wb'))
