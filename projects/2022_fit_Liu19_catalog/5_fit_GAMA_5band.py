...
#%%
# for _, line in enumerate(lines[1:100]):
# for _, line in enumerate(lines[100:200]):
# for _, line in enumerate(lines[200:300]):
for _, line in enumerate(lines[300:]):    
    ID, Ra, Dec = line.split(',')
    print(_, ID)
    QSO_RA, QSO_DEC = float(Ra), float(Dec)
    data_process_list = []
    for i, band in enumerate(bands):
        img_globname = glob.glob('GAMA_online_data_download/{0}/*cutout*-{1}-*.fits'.format(ID,band) )
        psf_globname = glob.glob('GAMA_online_data_download/{0}/*psf*-{1}-*.fits'.format(ID,band) )
    ...
    for i in run_list:  
        ...
        fit_sepc_l[i].plot_fitting_sets('fit_result_GAMA/'+ID+'-{0}_set.png'.format(band))
        fit_sepc_l[i].build_fitting_seq()
        fit_run_l[i] = FittingProcess(fit_sepc_l[i], savename = 'fit_result_GAMA/'+ID+'-{0}'.format(band), fitting_level=['norm', 'deep'])
        fit_run_l[i].run(algorithm_list = ['PSO', 'PSO'])
        fit_run_l[i].plot_final_qso_fit(save_plot=True, target_ID= 'fit_result_GAMA/'+ID +'-'+ band )
        fit_run_l[i].dump_result()
    print(_, ID, 'fitting finished')
