...
for i, band in enumerate(bands):
    img_globname = glob.glob('type2_online_data_download/{0}/*cutout*-{1}-*.fits'.format(ID,band) )
    psf_globname = glob.glob('type2_online_data_download/{0}/*psf*-{1}-*.fits'.format(ID,band) )
...
for i in run_list:  
    ...
    fit_sepc_l[i].plot_fitting_sets(savename='fit_result_type2/'+ID+'-{0}_set.png'.format(band), show_plot=False)
    fit_sepc_l[i].build_fitting_seq()
    fit_run_l[i] = FittingProcess(fit_sepc_l[i], savename = 'fit_result_type2/'+ID+'-{0}'.format(band), fitting_level=['norm', 'deep'])
    fit_run_l[i].run(algorithm_list = ['PSO', 'PSO'])
    fit_run_l[i].plot_final_qso_fit(save_plot=True, target_ID= 'fit_result_type2/'+ID +'-'+ band, show_plot=False)
