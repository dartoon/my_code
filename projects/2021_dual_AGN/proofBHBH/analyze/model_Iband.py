...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar  1 10:30:13 2021
...
import shutil
files = glob.glob('../Iband_data/*HSC-I.fits')
files.sort()

run_list = [[100,200], [200,300], [300,400], [400,500],[500,600],[600, 700], [700, 745]][6]

for run_i in range(run_list[0], run_list[1]):
    # run_i = 0
    file = files[run_i]
    ID = file.split('/')[2].split('_HSC')[0]
    RA_h = ID[:9]
    RA_h = RA_h[:2]+':'+RA_h[2:4] + ':' + RA_h[4:]
    Dec_h = ID[9:]
    Dec_h = Dec_h[:3]+':'+Dec_h[3:5] + ':' + Dec_h[5:]
    pos = SkyCoord('{0} {1}'.format(RA_h, Dec_h), unit=(u.hourangle, u.deg))
    image_RA, image_DEC = pos.ra.degree, pos.dec.degree
    ...
    image_folder = '../Iband_data/'
    fit_folder =  '../model_Iband/' + image_ID + '/'
    
    print(fit_folder)
    if os.path.exists(fit_folder)==False:
        os.mkdir(fit_folder)
    
    shutil.copy('./model_Iband.py', fit_folder)
    band_seq = ['I']
    run_list = [0]
    ...
    for i in range(len(band_seq)):
        ...
        try:
            data_process_i.generate_target_materials(radius=None, radius_list = [15, 20, 25, 30, 35, 40],
    ...
    for k in run_list:  #Search for Dual Based on I band.
        ...
        comp_mag = [round(source_result_2[i]['magnitude'],2) for i in range(len(source_result_2))]
        comp_x = [round(source_result_2[i]['center_x'],3) for i in range(len(source_result_2))]
        comp_y = [round(source_result_2[i]['center_y'],3) for i in range(len(source_result_2))]
        ...
        write_result.write("\nHost mag: "+repr(round(host_mag,3)))
        write_result.write("\nComp mag: "+repr(comp_mag)[1:-1] )
        write_result.write("\nComp x: "+repr(comp_x)[1:-1] )
        write_result.write("\nComp y: "+repr(comp_y)[1:-1] )
        write_result.write("\nAGN mag: ")
