...
image_folder = '../allband_data/'
# image_folder = 'extra_interesting/'
f = open("./proof-BHBH.txt","r")
string = f.read()
lines = string.split('\n')   # Split in to \n

files = glob.glob(image_folder+'*I.fits')
files.sort()
...
for run_i in range(run_idx, run_idx+1):
    ...
    print("Which ID:", run_i, image_ID)
    line = [lines[i] for i in range(len(lines)) if image_ID in lines[i]]
    _, image_RA, image_DEC, _ = line[0].split(' ')
    ...
    # ps_param1 = ps_params_generator([[-1,1], [2,-1]], [30,30], deltaPix = data_process_list[0].deltaPix)
    # ps_param2 = ps_param1
    #%%
