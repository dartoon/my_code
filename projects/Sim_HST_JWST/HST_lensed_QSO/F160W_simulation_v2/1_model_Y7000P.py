...
savename = '_result_PSFinter_subg3.pkl'
...
# savename = 'result_useDrzRms_subg3_psferr001_addspecial_noJoinSource.pkl' #+ Simon's points; PSF not change, psf_error_map 0.1
...
##        solver_type = 'THETA_E_PHI'
    ...
    if glob.glob(save_pkl_folder+ID+savename) == []:    
...
#            fermat_pot = td_cosmo.fermat_potential(kwargs_result['kwargs_lens'], kwargs_result['kwargs_ps'])
        ...
        pickle.dump([multi_band_list, kwargs_model, kwargs_result, chain_list, fix_setting, mcmc_new_list], open(save_pkl_folder+ID+savename, 'wb'))
    #%%Print fitting result:
    multi_band_list, kwargs_model, kwargs_result_best, chain_list, fix_setting, mcmc_new_list = pickle.load(open(save_pkl_folder+ID+savename,'rb'))
    # fixed_lens, fixed_source, fixed_lens_light, fixed_ps, fixed_cosmo = fix_setting
    labels_new = [r"$\gamma$", r"$D_{\Delta t}$","H$_0$" ]
