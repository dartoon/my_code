...
#            zp = [28., 27.9, 26.7, 27.75][filt_i]   #Using ETC, (616-556) total flux for 23.5 ab mag objects.  #Need to check
            ...
            # point_mag = 21 #qso_info['ID'+repr(ID)]['AGN_{0}_mag'.format(filt)]
            host_mag = 28.0  #qso_info['ID'+repr(ID)]['galaxy_{0}_mag'.format(filt)]
            ...
            point_flux =  host_flux * 1
            host_flux = point_flux*0.1
            
            host_ratio = host_flux/(host_flux+point_flux)
            if count == 0:
                if host_ratio< 0.1:
