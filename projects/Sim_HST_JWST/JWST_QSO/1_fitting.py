...
seed_l, seed_h = [[301, 311], [311, 321], [321, 331], [331, 341], [341, 351]][4]

for seed in range(seed_l, seed_h):
    for ID in [3]:#range(1, 7):
        for filt_i in [0, 3]:
            ...
            if glob.glob(folder+'/{0}.pkl'.format(save_name)) !=[]:
                ...
                for check_seed in range(seed_l, seed_h):
                    ...
                    for framesize in range(61, len(QSO_img_org_i), 10):
                        ...
                        peak = np.where(QSO_img_org_i==QSO_img_org_i.max())
                        peak = [peak[0][0], peak[1][0]]
                        QSO_img_i = QSO_img_org_i[peak[0]-half_r:peak[0]+half_r+1,peak[1]-half_r:peak[1]+half_r+1]
                        edges_value = np.concatenate((QSO_img_i[0,:], QSO_img_i[-1,:], QSO_img_i[0,:], QSO_img_i[-1,:]))
                        if np.median(edges_value)*5-np.std(edges_value)<0:
                            break
                    framesize_list.append(framesize)   
                framesize = np.max(framesize_list)
                print(framesize)                    
