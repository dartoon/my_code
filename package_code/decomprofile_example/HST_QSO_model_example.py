...
data_process = DataProcess(fov_image = fov_image, target_pos = [1142., 637.], header = header,
                          rm_bkglight = False, exptime = exp_map, if_plot=False, zp = 27.0)
data_process.generate_target_materials(radius=55, create_mask = False, nsigma=2.8,
...
del fit_sepc.kwargs_model['source_light_model_list']  #Remove all the aperture and fit only PS.

fit_sepc.build_fitting_seq()

#%%Setting the fitting method and run.
from decomprofile.fitting_process import FittingProcess
fit_run = FittingProcess(fit_sepc, savename = 'test_no_host')
fit_run.run()
fit_run.plot_all()
fit_run.dump_result()
